...
- `src/lib/email.js` sends low-stock alerts via Resend and is a no-op (logs a warning, doesn't throw) if `RESEND_API_KEY`/`ALERT_EMAIL_TO` aren't set — the automatic trigger in `src/server.js` (`app.locals.onStockMovement`) only fires on the transition into low stock, not on every movement, to avoid spamming.
