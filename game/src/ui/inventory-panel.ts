...
import { assignToSlot, wearItem } from "../systems/equipment";
import { SLOT_NAMES, isWearable, slotFor } from "../data/worn";
...
export class InventoryPanel {
  ...
  private render(): void {
    ...
    // one worn row at the top of the bag was fine, but three would have pushed
    // the actual inventory down the panel to make room for mostly-empty slots.
    this.list.replaceChildren(
      ...this.state.inventory.map((slot) => {
        ...
        // Worn gear gets its own verb next to Hold rather than sharing one,
        // and the button names the slot it will go into — with three of them,
        // a bare "Wear" leaves the player to guess what it displaces.
        const wornSlot = slotFor(def.id);
        if (wornSlot !== null) {
          const wear = el("button", undefined, `Wear · ${SLOT_NAMES[wornSlot]}`);
          wear.addEventListener("click", () => {
            wearItem(this.state, def.id);
            this.render();
          });
        ...
        //
        // Worn gear is the one thing with no Hold: it goes on the body, and a
        // quick slot holding it would be a slot that does nothing when pressed.
        if (!isWearable(def.id)) {
